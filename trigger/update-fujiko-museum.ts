...
import { main } from '@/scripts/update-fujiko-museum';
import { translateContentTask } from '@/trigger/translate-content';

export const updateFujikoMuseumTask = schedules.task({
  id: 'update-fujiko-museum',
  cron: { pattern: '30 0,6-23 * * *', timezone: 'Asia/Tokyo' },
  maxDuration: 300,
  run: async (payload) => {
    logger.log('Starting scheduled fujiko-museum update', {
    ...
    try {
      ...
      logger.log('Fujiko-museum update completed', {
      ...
      logger.error('Fujiko-museum update failed', { error });
      throw error;
    }
  },
});
